...
from utils.util import draw_attention_map, draw_counting_map
    ...
    def forward(self, images, labels, name, is_train=False):
        ...
        if self.draw_map:
            ...
            for i in range(images.shape[0]):
                # paddle仍然具有detach
                img = paddle.to_tensor(images[i][0].detach(), place=paddle.CPUPlace()).numpy() * 255
                # img = images[i][0].detach().cpu().numpy() * 255
                # draw attention_map
                for step in range(len(word_probs)):
                    word_atten = paddle.to_tensor(word_alphas[step][0].detach(), place=paddle.CPUPlace()).numpy() 
                ...
                for idx in range(self.out_channel):
                    ...
                    counting_map = paddle.to_tensor(paddle.transpose(counting_maps[0],[1,2,0])[:,:,idx].detach() , place=paddle.CPUPlace())
...
class AttDecoder(nn.Layer):
    ...
    def forward(self, cnn_features, counting_preds, is_train=False):
        ...
        batch_size, _, height, width = cnn_features.shape
        # image_mask = paddle.ones([batch_size, 1, height, width]).to(self.device)
        ...
        # word_alpha_sum = paddle.zeros((batch_size, 1, height, width)).to(device=self.device)
        word_alpha_sum = paddle.zeros([batch_size, 1, height, width])
        hidden = self.init_hidden(cnn_features, image_mask)
        # word_embedding = self.embedding(paddle.ones([batch_size]).long().to(device=self.device))
        word_embedding = paddle.floor(self.embedding(paddle.ones([batch_size])))
